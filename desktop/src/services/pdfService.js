...
export async function createPdfFromReport(report) {
  ...
  // 3 columns only: Finding | Count | AI Confidence (no colour swatch)
  const tCols = [CW*0.50, CW*0.22, CW*0.28]
  ...
  rows.forEach(({ label, key }, ri) => {
    ...
    let tx2 = ML
    page.drawText(safe(label), { x:tx2+4, y:y-9, size:8, font:bold, color:NAVY })
    tx2 += tCols[0]
    page.drawText(safe(count), { x:tx2+4, y:y-9, size:8, font:bold, color:TEAL })
    tx2 += tCols[1]
    page.drawText(safe(conf),  { x:tx2+4, y:y-9, size:8, font, color:GRAY })
    y -= 14
  })

  // AI Confidence disclaimer in PDF
  checkY(28)
  page.drawRectangle({ x:ML, y:y-24, width:CW, height:27, color:rgb(1,0.98,0.94),
    borderColor:rgb(0.85,0.65,0.1), borderWidth:0.5 })
  page.drawText('AI Confidence Disclaimer:', { x:ML+5, y:y-10, size:7.5, font:bold, color:rgb(0.6,0.4,0) })
  const disc2 = 'Values are auto-generated by the AI model and may improve with subsequent training iterations. For clinical reference only.'
  const d2words = disc2.split(' '); const d2lines = []; let d2c = ''
  for (const w of d2words) {
    const t = d2c ? d2c+' '+w : w
    if (font.widthOfTextAtSize(t,7) < CW-12) d2c = t
    else { d2lines.push(d2c); d2c = w }
  }
  if (d2c) d2lines.push(d2c)
  d2lines.forEach((l,i) => {
    page.drawText(l, { x:ML+5, y:y-18-(i*9), size:7, font, color:rgb(0.55,0.38,0.02) })
  })
  y -= 30
